    ...
    <div style={{ animation: 'fadeIn 0.25s ease' }}>
      ...
      {exercises.length > 0 && (
        <div className="glass-panel" style={{ padding: '1.25rem', marginBottom: '1.75rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', alignItems: 'center' }}>
            ...
            <div>
              <select className="input-field" value={selectedMuscle} onChange={(e) => setSelectedMuscle(e.target.value)}>
            ...
            <div>
              <select className="input-field" value={selectedEquipment} onChange={(e) => setSelectedEquipment(e.target.value)}>
          ...
          {filtered.map((ex) => {
            ...
            return (
              ...
              >
                <div>
                  {ex.image && (
                    <div style={{ width: '100%', height: '140px', borderRadius: 'var(--radius-sm)', overflow: 'hidden', marginBottom: '0.85rem' }}>
        ...
        <div className="modal-overlay" onClick={() => setShowAddModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
              <h3 style={{ fontSize: '1.25rem', fontWeight: 700 }}>Add New Exercise</h3>
            ...
            <form onSubmit={handleCreateExercise}>
              <div style={{ marginBottom: '1rem' }}>
                ...
                <input
                  type="text"
                  className="input-field"
                  placeholder="e.g. Barbell Squat, Lat Pulldown..."
                  value={name}
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                ...
                {image ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <img src={image} alt="Preview" style={{ width: '60px', height: '60px', borderRadius: 'var(--radius-sm)', objectFit: 'cover' }} />
                    <button type="button" className="btn btn-secondary btn-sm" onClick={() => setImage(null)}>Remove Image</button>
                  </div>
                ) : (
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                ...
                <input
                  type="text"
                  className="input-field"
                  placeholder="e.g. Pause at bottom for 1 sec..."
                  value={notes}
      ...
      {selectedExerciseDetail && (
        <div className="modal-overlay" onClick={() => setSelectedExerciseDetail(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            {selectedExerciseDetail.image && (
              <div style={{ width: '100%', height: '180px', borderRadius: 'var(--radius-sm)', overflow: 'hidden', marginBottom: '1rem' }}>
                <img src={selectedExerciseDetail.image} alt={selectedExerciseDetail.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
            )}
            ...
            <h4 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: '0.75rem' }}>Performance History</h4>
                  ...
                  return (
                    <div key={w.id} style={{ background: 'rgba(255,255,255,0.03)', padding: '0.65rem 0.85rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)' }}>
                      ...
                      <div style={{ fontWeight: 600, fontSize: '0.85rem', color: '#fff' }}>
                        Sets: {exData.sets.map(s => `${s.weight || 0}kg × ${s.reps || 0}`).join(' | ')}
                      </div>
                    </div>
                  );
                })
